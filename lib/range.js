...
function Range(){
  ...
  // instance properties
  var instance_properties = {start: ilow, length: n, step: istep};
  Object.keys(instance_properties).forEach(function(key){
    Object.defineProperty(me, key, {
      value: instance_properties[key],
    });
  });
};
...
Range.prototype.indexOf = function(index){
...
function forEach(range, callback, _this, _internal_reverse){
  if(_internal_reverse){
    for(var i=range.length-1; i>-1; i--){
      callback.call(_this, range.indexOf(i), i, range);
    }
  }else{
...
function reduce(_internal_reverse, range, callback, initialValue){
  var value
    , isValueSet = false;
